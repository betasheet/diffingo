...
class Printer {
 public:
   ...
   */
  explicit Printer(const Style &style);
   ...
   *
   * setIndentLabels(true) + setLabelsDefineIndent(false) -> label at same level
   *as contents
   * setIndentLabels(false) + setLabelsDefineIndent(false) -> Qt coding style
   * setIndentLabels(true) + setLabelsDefineIndent(true) -> kdepim coding style,
   *default here.
   * setIndentLabels(false) + setLabelsDefineIndent(true) -> doesn't really make
   *sense.
   */
   ...
   */
  std::string functionSignature(const Function &function,
 ...
 protected:
   ...
   */
  virtual std::string creationWarning() const;
   ...
   * header.
   */
  virtual std::string licenseHeader(const File &file) const;
